...
1. Install and open the APK while online once; then enable airplane mode and complete Quick demo through `.nc` export.
...
5. Submit separate in-app reports for each workflow issue. Use **Toolpath** or **Export** for machining-affecting requests so automation holds them for review.
