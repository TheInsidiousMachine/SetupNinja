...
export function loadWorkerConfig(env = process.env) {
  ...
  return {
    ...
    autoRelease: env.SETUPNINJA_AUTO_RELEASE === "true",
    releaseRoot: path.resolve(env.SETUPNINJA_RELEASE_ROOT ?? path.join(AUTOMATION_ROOT, "runtime", "releases")),
    releaseBaseUrl: env.SETUPNINJA_RELEASE_BASE_URL,
    updateSigningKeyFile: env.SETUPNINJA_UPDATE_SIGNING_KEY_FILE,
    updatePublicKeyFile: env.SETUPNINJA_UPDATE_PUBLIC_KEY_FILE,
    feedbackUrl: env.SETUPNINJA_FEEDBACK_URL,
    updateUrl: env.SETUPNINJA_UPDATE_URL,
    androidHome: env.ANDROID_HOME,
    expectedCertificateSha256: env.SETUPNINJA_ANDROID_CERT_SHA256,
